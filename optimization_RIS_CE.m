function [optimal_power_ul, optimal_rate_up, freq_MEH, power_mec, opt_conf, power_RIS, opt_beam] = ...
    ...
    [opt_conf, opt_beam] = greedy_optimization_ce(current_conf, N_blocks, N, possible_confs, ...
           ...
           N0, B_u, V, Q_tilde_up, p_bit, alpha, possible_angles, weights_comb);
    ...
    channel_over_noise_ul = (abs(weights_comb(:, opt_beam)' * (direct_channel + squeeze(pagemtimes(permute(reflected_channel, [2, 3, 1]), opt_conf)))) .^2 / (N0 * B_u)).';       
    ...
    for ll = 1:N_blocks
        ...
        for vv = 1:length(possible_confs)           
            ...
            for bb = 1:possible_angles
                ...
                channel_over_noise = (abs(weights_comb(:, bb)' * (direct_channel + squeeze(pagemtimes(permute(reflected_channel, [2, 3, 1]), try_conf)))) .^2 / (N0 * B_u)).';       
            ...
            % transposition to obtain a K x 1 vector in the end            
            channel_over_noise = (abs(weights_comb(:, opt_angle)' * (direct_channel + squeeze(pagemtimes(permute(reflected_channel, [2, 3, 1]), try_conf)))) .^2 / (N0 * B_u)).';
