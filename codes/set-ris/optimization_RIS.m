%%% OPTIMIZATION %%%
function [power_up, rate_up, freq_MEH, power_mec, current_v, power_RIS] = ...
    optimization_RIS( ...
    ...
    for kk = 1:K
        ...
        % Uplink 
        rate_up(kk) = min(B_u * log2(1 + channel_over_noise_up(kk) .* Pt(kk)), Q_local(kk) / delta);
        p_(kk) = 1 ./ channel_over_noise_up(kk) .* (exp(rate_up(kk) * log(2) / B_u) - 1);
        power_up(kk,1) = min(max(0, Q_tilde_up(kk) / (alpha * V * log(2)) - 1 ./ (channel_over_noise_up(kk))), p_(kk));
    end
    power_up(Q_tilde_up<=0) = 0;
    rate_up = B_u * log2(1 + channel_over_noise_up .* power_up);
