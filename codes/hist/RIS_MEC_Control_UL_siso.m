...
f = 28e9;               % Frequency
overall_channel_user_RIS_up = zeros(N_RIS, K, N_slot); overall_channel_RIS_AP_up = overall_channel_user_RIS_up; overall_channel_AP_up = zeros(K, N_slot);
for qq = 1:N_slot
        [Hdirt, H1t, H2t] = channel_mat_RIS(Nt, Nr, N_RIS, lt, lr, D, K, rice_fac, f, dist_ris, alpha_dir);
...
%%% CHANNEL %%%
function [Hdir, H1, H2] = channel_mat_RIS(Nt, Nr, Nris, lt, lr, D, no_mat, K, f, dist_ris, varargin)
% data
...
function [optimal_power_up, optimal_rate_up, freq_MEH, power_mec, current_v, power_RIS] = optimization_RIS(K, B_u,...
    Q_local, Q_MEH, Z, qtz_RIS, N_RIS, N_blocks, channel_RIS_up, channel_RIS_AP_up, channel_AP_up,...
    N0, V, Pt, possible_f, J, delta, p_bit, kappa, alpha)
    ...
    proba = 0.9;
    toss_coins = rand(K, 1);
    config_v = dftmtx(N_RIS);

    overall_channel_RIS_up = zeros(K, 1); channel_over_noise_up = zeros(K, 1);
    for kk = 1:K
        
        %if toss_coins(kk) < proba
        overall_channel_RIS_up(kk) = overall_channel_RIS_up(kk) + (channel_RIS_up(:, kk).' * config_v * channel_RIS_AP_up(:, kk));
        %else
